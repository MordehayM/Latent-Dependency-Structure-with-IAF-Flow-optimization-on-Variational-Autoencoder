...
def load_binary_mnist(cfg, **kwcfg):
    ...
    f = h5py.File(pathlib.os.path.join(fname), 'r')
    x_train = f['train'][::]
    x_val = f['valid'][::]
    x_test = f['test'][::]
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"Creation of the directory {cfg.my_train} failed")
    else:
        print(f"Successfully created the directory {cfg.my_train} ")
    ...
    #riter.add_graph(model,(torch.rand((1, 128),device=device), torch.rand((1, 784),device=device)))
    #writer.add_graph(variational, torch.rand((1, 784), device=device))
    ...
    for step, batch in enumerate(cycle(train_data)):
        ...
        if step % cfg.log_interval == 0:
            images = torchvision.utils.make_grid(x.reshape(-1, 1, 28, 28))
            ...
            writer.add_scalars('Elbo', {'train': elbo.detach().cpu().numpy().mean(), 'val':valid_elbo}, step)
            writer.add_scalar('Log p(x)', valid_log_p_x, step)
            train_elbo.append(elbo.detach().cpu().numpy().mean())
            val_elbo.append(valid_elbo)
            Log_Val_p_x.append(valid_log_p_x)
            ...
            if num_no_improvement > cfg.early_stopping_interval:
                ...
                writer.add_scalar('elbo', test_elbo, step)
                writer.add_scalar('log p(x)', test_log_p_x, step)
                writer.close()
                train_elbo = np.array(train_elbo)
                val_elbo = np.array(val_elbo)
                Log_Val_p_x = np.array(Log_Val_p_x)
                np.savez(f"Elbo", train=train_elbo, val=val_elbo)
                np.savez(f"Log_Val_p_x", Log_p_x=Log_Val_p_x)

                # save plot
                plt.plot(np.arange(0, cfg.log_interval*len(train_elbo), cfg.log_interval), train_elbo,
                          np.arange(0, cfg.log_interval*len(val_elbo), cfg.log_interval), val_elbo,
                         np.arange(0, cfg.log_interval*len(val_elbo), cfg.log_interval), Log_Val_p_x)
                plt.xlabel('Step')
                plt.ylabel('Metric')
                plt.title('Metric Vs Step')
                plt.grid(True)
                plt.legend(['train_elbo', 'val_elbo', 'Log_p(x)_val'], loc='upper right')
                plt.savefig('Metric_Vs_Step.jpg')
                break

'''
elbo = np.load('Elbo.npz')
p_x = np.load('Log_Val_p_x.npz')
plt.plot(np.arange(20000, cfg.log_interval*len(elbo['train']), cfg.log_interval), elbo['train'],
                          np.arange(20000, cfg.log_interval*len(elbo['val']), cfg.log_interval), elbo['val'],
                         np.arange(20000, cfg.log_interval*len(p_x['Log_p_x']), cfg.log_interval), p_x['Log_p_x'])
plt.xlabel('Step')
plt.ylabel('Metric')
plt.yscale('log')
plt.title('Metric Vs Step')
